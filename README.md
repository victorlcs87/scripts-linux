...
| `16` | Backup e restore de configuracoes | Faz backup so das **configuracoes** (nao dos apps) dos programas que o Reforja instala num `.tar.gz`, opcionalmente **cifrado com senha** (gpg AES256, recomendado pois guarda tokens e sessoes). Grava em `~/GoogleDrive/reforja-backups` quando o Drive esta montado, senao em `~/reforja-backups`; exclui caches, saves de jogos e perfis pesados, deixa escolher quais apps incluir e mostra o tamanho estimado. O restore pergunta o caminho do backup (util quando o rclone ainda nao esta configurado), pede a senha se cifrado, deixa escolher quais apps restaurar, verifica a integridade, pede confirmacao digitada e faz copia de seguranca **local** das configs atuais antes de sobrescrever. Um item extra limpa backups antigos mantendo os mais recentes. |
