...
| `06` | Git / GitHub | Menu simples: instala Git + GitHub CLI (gh), conecta a conta pelo navegador (o gh cria/envia a chave SSH), adiciona repositorios (clona em ~/repositorios, escolhendo da sua lista, e ja configura o autor dos commits) e configura host alias SSH por conta para separar 2+ contas (git@<alias>:owner/repo.git). Guarda o que foi configurado em ~/.config/reforja/git.json. |
